...
export const getConfigSource = (): ConfigSource => {
  ...
  logInfo(
    `You can supply either --config-file or --config-variable to specify a different configuration.`,
  )
  return { type: 'file', path: path }
}
  ...
  if (source.type === 'file') {
    try {
      ...
      if (checkExists) {
        ...
        logInfo(
          "You may want to run `mintable setup` (or `mintable migrate`) if you haven't already.",
        )
      }
    }
  }
