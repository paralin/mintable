...
export default async () => {
  ...
  let endDate = config.transactions.endDate
    ? parseISO(config.transactions.endDate)
    : new Date()
  ...
  for (const accountId in config.accounts) {
    ...
    logInfo(
      `Fetching account ${accountConfig.id} using ${accountConfig.integration}.`,
    )
    ...
    switch (accountConfig.integration) {
      case IntegrationId.CSVImport:
        ...
        accounts = accounts.concat(
          await csv.fetchAccount(accountConfig, startDate, endDate),
        )
        break
      ...
      case IntegrationId.Teller:
        ...
        accounts = accounts.concat(
          await teller.fetchAccount(accountConfig, startDate, endDate),
        )
        break
  ...
  const numTransactions = () =>
    accounts
      .map((account) =>
        account.hasOwnProperty('transactions')
    ...
    return rule.conditions
      .map((condition) =>
        new RegExp(condition.pattern, condition.flags).test(
          transaction[condition.property],
        ),
      )
  ...
  if (config.transactions.rules) {
        ...
        .map((transaction) => {
          config.transactions.rules.forEach((rule) => {
            if (transaction && transactionMatchesRule(transaction, rule)) {
              ...
              if (
                rule.type === 'override' &&
                transaction.hasOwnProperty(rule.property)
              ) {
                transaction[rule.property] = (
                  transaction[rule.property].toString() as String
                ).replace(
    ...
    logInfo(
      `${numTransactions()} transactions out of ${totalTransactions} total transactions matched filters.`,
    )
    logInfo(
      `${countOverridden} out of ${totalTransactions} total transactions overridden.`,
    )
  }
