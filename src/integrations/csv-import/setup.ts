import {
  defaultCSVImportConfig,
  CSVImportConfig,
} from '../../types/integrations/csv-import'
...
export default async () => {
  return new Promise<void>(async (resolve, reject) => {
    try {
      ...
      console.log(
        '\t2. Copy the absolute path of this folder (globs for multiple files are also supported).',
      )
      ...
      const responses = await prompts([
        ...
        },
        {
          ...
          name: 'path',
          message:
            "What is the path/globs to the CSV file(s) you'd like to import?",
      ...
      updateConfig((config) => {
        let CSVImportConfig =
          (config.integrations[IntegrationId.CSVImport] as CSVImportConfig) ||
          defaultCSVImportConfig
