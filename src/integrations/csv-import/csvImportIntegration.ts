...
export class CSVImportIntegration {
  ...
  constructor(config: Config) {
    this.config = config
    this.CSVImportConfig = this.config.integrations[
      IntegrationId.CSVImport
    ] as CSVImportConfig
  }
  ...
  public fetchAccount = async (
    ...
    return new Promise(async (resolve, reject) => {
            ...
            return files.map((match) => {
              try {
                ...
                const transactions: Transaction[] = rows.map((inputRow) => {
                  ...
                  Object.keys(CSVAccountConfig.transformer).map(
                    (inputColumn) => {
                      // Concatenate multiple columns
                      if (inputColumn.includes('+')) {
                        outputRow[
                          CSVAccountConfig.transformer[inputColumn] as string
                        ] = inputColumn
                          .split('+')
                          .map((c) => inputRow[c])
                          .join(' - ')
                  ...
                  if (
                    CSVAccountConfig.negateValues === true &&
                    outputRow.hasOwnProperty('amount')
                  ) {
      ...
      logInfo(
        `Successfully imported transactions for integration ${IntegrationId.CSVImport}`,
        account,
      )
