...
export default async () => {
  try {
    ...
    console.log(
      '\t4. Find the application ID, and download the certificate and private key',
    )
    ...
    const credentials = await prompts([
      ...
      },
      {
        ...
        validate: (s: string) =>
          s.length && existsSync(s)
            ? true
            : 'Must enter path to certificate file.',
      },
      {
        type: 'text',
        name: 'pathPrivateKey',
        ...
        validate: (s: string) =>
          s.length && existsSync(s)
            ? true
            : 'Must enter path to private key file.',
      },
      {
        type: 'text',
        name: 'appId',
        ...
        validate: (s: string) =>
          s.length && s.startsWith('app_')
            ? true
            : 'Must enter Application ID from Teller.',
      },
    ])

    updateConfig((config) => {
      const tellerConfig =
        (config.integrations[IntegrationId.Teller] as TellerConfig) ||
        defaultTellerConfig
