...
import {
  TellerConfig,
  TellerTransaction,
} from '../../types/integrations/teller'
...
export class TellerIntegration {
  ...
  constructor(config: Config) {
    this.config = config
    this.tellerConfig = this.config.integrations[
      IntegrationId.Teller
    ] as TellerConfig
  }
  ...
  public accountSetup = (): Promise<void> => {
    return new Promise((resolve) => {
      const app = express()
        ...
        .use(
          express.static(path.resolve(path.join(__dirname, '../../../docs'))),
        )
      ...
      app.post('/get_enrollment_accounts', async (req, res) => {
        if (!req.body.enrollment || !req.body.enrollment.accessToken) {
          logError(
            'Received invalid request body for /get_enrollment_accounts',
            req.body,
          )
          res.status(401)
          return res.json({})
        ...
        const accounts = await this.tellerApi(
          req.body.enrollment.accessToken,
          'accounts',
        )
      ...
      app.post('/register_account', (req, res) => {
        if (!req.body.accessToken || !req.body.accountId) {
          logError(
            'Received invalid request body for /register_account',
            req.body,
          )
          res.status(401)
          return res.json({})
      ...
      app.post('/accounts', async (req, res) => {
        ...
        for (const accountId in this.config.accounts) {
          const accountConfig: TellerAccountConfig = this.config.accounts[
            accountId
          ] as TellerAccountConfig
          if (accountConfig.integration === IntegrationId.Teller) {
            const accountInfo = await this.tellerApi(
              accountConfig.token,
              `accounts/${accountConfig.id}`,
            )
            accounts.push({
              id: accountConfig.id,
              name: accountInfo
                ? `${accountInfo.institution?.name} ${accountInfo.name} ${accountInfo.last_four}`
                : '-',
      ...
      app.get('/', (req, res) =>
        res.sendFile(
          path.resolve(
            path.join(
              __dirname,
              '../../../src/integrations/teller/account-setup.html',
            ),
          ),
        ),
      )
    ...
    return new Promise((resolve, reject) => {
      ...
      try {
        return this.tellerApi(
          accountConfig.token,
          `accounts/${accountConfig.id}/transactions`,
        ).then((ttxs: TellerTransaction[]) => {
          if (!ttxs.reduce) {
            logError(
              'Received unexpected data from Teller transactions API',
              ttxs,
            )
            throw new Error(
              'Received unexpected data from Teller transactions API',
          ...
          const filteredTtxs: TellerTransaction[] = ttxs.reduce(
            (results, ttx) => {
              const ttxDate = parseISO(ttx.date)
              if (
                compareAsc(ttxDate, startDate) !== -1 &&
                compareAsc(ttxDate, endDate) !== 1
              ) {
                results.push(ttx)
              }
              return results
          ...
          logInfo(
            `Received ${ttxs.length} transactions. Will import ${filteredTtxs.length} transactions within date range.`,
          )
  ...
  public fetchAccount = async (
    ...
    if (startDate < subMonths(new Date(), 5)) {
      logWarn(
        'Transaction history older than 6 months may not be available for some institutions.',
        {},
      )
    }
    ...
    if (accountInfo.error) {
      logWarn(
        'Could not fetch data for account from Teller. Authentication may be expired. Run: `mintable teller-account-setup`',
        {},
      )
    ...
    const accountBalance = await this.tellerApi(
      accountConfig.token,
      `accounts/${accountConfig.id}/balances`,
    )
    ...
    const data = await this.fetchPagedTransactions(
      accountConfig,
      startDate,
      endDate,
    )
    ...
    if (!data || !data.map) {
      logWarn(
        `fetchAccount received unexpected response for transactions from ${account.institution} ${account.account}`,
        data,
      )
      return []
    }
