...
export default () => {
  return new Promise((resolve, reject) => {
    try {
      ...
      console.log(
        '\t2. Sign in with your banking provider for each account you wish to link.',
      )
      console.log(
        "\t3. Click 'Done Linking Accounts' in your browser when you are finished.\n",
      )

      const config = getConfig()
      const tellerConfig = config.integrations[
