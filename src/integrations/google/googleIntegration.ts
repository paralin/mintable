...
export class GoogleIntegration {
  ...
  constructor(config: Config) {
    this.config = config
    this.googleConfig = config.integrations[
  ...
  public saveAccessTokens = (tokens: Credentials): void => {
    updateConfig((config) => {
      let googleConfig = config.integrations[
        IntegrationId.Google
      ] as GoogleConfig
  ...
  public getSheets = (
    ...
    return this.sheets
      ...
      .catch((error) => {
        logError(
          `Error fetching sheets for spreadsheet ${this.googleConfig.documentId}.`,
          error,
        )
        return []
      })
  ...
  public copySheet = async (
    title: string,
    sourceDocumentId?: string,
  ): Promise<sheets_v4.Schema$SheetProperties> => {
    const sheets = await this.getSheets(
      sourceDocumentId || this.googleConfig.documentId,
    )
    let sourceSheetId

    try {
      sourceSheetId = sheets.find((sheet) => sheet.properties.title === title)
        .properties.sheetId
    } catch (error) {
      logError(
        `Error finding template sheet ${title} in document ${sourceDocumentId}.`,
        { error, sheets },
      )
    }
  ...
  public renameSheet = async (
    ...
    const sheets = await this.getSheets()
    const sheetId = sheets.find((sheet) => sheet.properties.title === oldTitle)
      .properties.sheetId
  ...
  public translateRanges = (ranges: Range[]): string[] =>
    ranges.map(this.translateRange)
  ...
  public clearRanges = (
    ...
    return this.sheets.values
      ...
      .then((res) => {
        logInfo(
          `Cleared ${ranges.length} range(s): ${translatedRanges}.`,
          res.data,
        )
        return res.data
      })
      .catch((error) => {
        logError(
          `Error clearing ${ranges.length} range(s): ${translatedRanges}.`,
          error,
        )
        return {}
      })
  ...
  public updateRanges = (
    ...
    return this.sheets.values
      ...
      .then((res) => {
        logInfo(
          `Updated ${data.length} range(s): ${data.map((r) => r.range)}.`,
          res.data,
        )
        return res.data
      })
      .catch((error) => {
        logError(
          `Error updating ${data.length} range(s): ${data.map((r) => r.range)}.`,
          error,
        )
        return {}
      })
  ...
  public sortSheets =
    async (): Promise<sheets_v4.Schema$BatchUpdateSpreadsheetResponse> => {
      const sheets = await this.getSheets()
  ...
  public formatSheets =
    async (): Promise<sheets_v4.Schema$BatchUpdateSpreadsheetResponse> => {
      ...
      return this.sheets
        ...
        .catch((error) => {
          logError(
            `Error updating formatting for ${sheets.length} sheets.`,
            error,
          )
          return {}
        })
  ...
  public updateSheet = async (
    ...
    const data = [columns].concat(
      rows.map((row) => this.getRowWithDefaults(row, columns)),
    )
  ...
  public updateTransactions = async (accounts: Account[]) => {
    ...
    const transactions = sortBy(
      accounts.map((account) => account.transactions).flat(10),
      'date',
    )
    ...
    console.log(
      `https://docs.google.com/spreadsheets/d/${this.googleConfig.documentId}`,
    )
  }
  ...
  public updateBalances = async (accounts: Account[]) => {
    ...
    await this.updateSheet(
      'Balances',
      accounts,
      this.config.balances.properties,
    )
