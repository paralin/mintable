import {
  GoogleConfig,
  defaultGoogleConfig,
} from '../../types/integrations/google'
...
export default async () => {
  return new Promise<void>(async (resolve, reject) => {
    try {
      ...
      const credentials = await prompts([
        {
          ...
          validate: (s: string) =>
            0 < s.length && s.length <= 64
              ? true
              : 'Must be between 0 and 64 characters in length.',
        },
        {
          type: 'password',
          name: 'clientId',
          message: 'Client ID',
          validate: (s: string) =>
            s.length >= 8 ? true : 'Must be at least 8 characters in length.',
        },
        {
          type: 'password',
          name: 'clientSecret',
      ...
      updateConfig((config) => {
        let googleConfig =
          (config.integrations[IntegrationId.Google] as GoogleConfig) ||
          defaultGoogleConfig
