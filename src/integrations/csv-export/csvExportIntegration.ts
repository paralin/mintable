...
export class CSVExportIntegration {
  ...
  constructor(config: Config) {
    this.config = config
    this.CSVExportConfig = this.config.integrations[
      IntegrationId.CSVExport
    ] as CSVExportConfig
  }

  public updateTransactions = async (accounts: Account[]) => {
    try {
      const transactions: Transaction[] = accounts
      ...
      const output = transactions.map((transaction) => ({
        ...
        date: format(
          transaction.date,
          this.CSVExportConfig.dateFormat || 'yyyy.MM',
        ),
      }))
      ...
      logError(
        `Error exporting transactions for integration ${IntegrationId.CSVExport}`,
        error,
      )
    }
  }
  ...
  public updateBalances = async (accounts: Account[]) => {
    try {
      ...
      logError(
        `Error exporting account balances for integration ${IntegrationId.CSVExport}`,
        error,
      )
    }
  }
